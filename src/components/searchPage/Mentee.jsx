...
import FilterMentee from './menteeFilter';
...
const Mentee = () => {
    ...
    function addSkill(){
        arrayOfDivs.push(<span  className='me-2 mt-2 d-inline-block  custom-padding rounded bg-secondaryColor-op text-white'>{inputValue}</span>)
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-md-3 col-12">
                            ...
                            {/* Check boxes */}
                            <div className={ mobileToggle ?"d-none d-sm-block mt-3":"d-sm-block mt-3"}>
                                <div className="form-check ps-0">
                                    <label className="form-check-label small " htmlFor="flexCheckDefault">Availlable for Hiring</label>
                                    <input className="form-check-input float-end"  onChange={()=>setAvaillable((Availlable)=>!Availlable)}    type="checkbox" id="flexCheckDefault"/>
                                </div>
                            </div>
                            ...
                            {/* Skills */}
                            <div className={mobileToggle ? "mt-5 d-none skills d-sm-block":"mt-5 skills d-sm-block"}>
                                <h5 className='small text-sec-color'>Skills</h5>
                                <p className='skillsField mb-2 text-gray'>{arrayOfDivs.length ? arrayOfDivs.map((item) => item) : ""}</p>
                                <input placeholder='Add Skill' type="text" value={inputValue}  onChange={(e)=>setInputValue((e.target.value).toLocaleLowerCase())}  className='outlint-focus'   />
                                <div>
                                    <button onClick={addSkill} className='d-inline-block mt-4 border-0 bg-secondaryColor-op text-white rounded'>Add</button>
                                    <button onClick={handleReset} className='d-inline-block mt-4 ms-4 border-0 bg-secondaryColor-op text-white rounded'>Reset</button>
                                </div>
                            </div>
                        </div>
                        {/* Mentors */}
                        <div className="col-md-9 col-12">
                           <div className='row'>
                                <FilterMentee Availlable={Availlable} locvalue={locvalue} skills={skills}/>
                           </div>
                        </div>
                    </div>
                </div>
